...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt_consulta) {
        ...
        if (mysqli_num_rows($resultado) > 0) {
            ...
            $atualizar_dados = "UPDATE onibus SET chassi = ?, placa = ?, cor = ?, assentos = ?, modelo = ? WHERE cpf = ?";
            $stmt_atualizar = mysqli_prepare($conexao, $atualizar_dados);
            if ($stmt_atualizar) {
                mysqli_stmt_bind_param($stmt_atualizar, "ssssss", $chassi, $placa, $cor, $assentos, $modelo, $cpf);
                ...
                if (mysqli_stmt_execute($stmt_atualizar)) {
                    ...
                    header("Location: http://localhost/site-onibus/motorista/cadastrar-rotas/index.php");
                    exit();
                } else {
            ...
            $inserir_dados = "INSERT INTO onibus (cpf, chassi, placa, cor, assentos,  modelo) VALUES (?, ?, ?, ?, ?, ?)";
            $stmt_inserir = mysqli_prepare($conexao, $inserir_dados);
            if ($stmt_inserir) {
                mysqli_stmt_bind_param($stmt_inserir, "ssssss", $cpf, $chassi, $placa, $cor, $assentos, $modelo);
